...
import (
	...
	"gitea.dev/backend/models/db"       //nolint:depguard // allow to access db in migration tests
	"gitea.dev/backend/models/unittest" //nolint:depguard // allow to access db in migration tests
	"gitea.dev/backend/modules/git"
	"gitea.dev/backend/modules/setting"
	"gitea.dev/backend/modules/testlogger"
...
func PrepareTestEnv(t *testing.T, skip int, syncModels ...any) (base.EngineMigration, func()) {
	...
	fixturesDir := filepath.Join(giteaRoot, "backend/modelmigration/fixtures", t.Name())
