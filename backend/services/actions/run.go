...
import (
	...
	"gitea.dev/backend/modules/actions/jobparser"
...
func InsertRun(ctx context.Context, run *actions_model.ActionRun, content []byte, vars map[string]string, inputs map[string]any, wfRawConcurrency *act_model.RawConcurrency) error {
	...
	if err := db.WithTx(ctx, func(ctx context.Context) error {
		...
		// so that run.ID is populated. Expressions referencing github.run_id —		// in run-name, job names, runs-on, or a workflow-level concurrency
...
// job-level concurrency, inserts it, and —for a ready no-needs reusable caller —// inline-expands (or skips) it. It returns the inserted job, any jobs cancelled by
