...
func viteDevModuleID(srcPath string) string {
	// frontend source files live under "frontend/" on disk, while their manifest/URL paths stay "web_src/..."
...
func IsViteDevRequest(req *http.Request) bool {
	...
	if strings.HasPrefix(path, "/@vite/") /* HMR client */ ||
		strings.HasPrefix(path, "/@fs/") /* out-of-root file access, see frontend/vite.config.ts: fs.allow */ ||
