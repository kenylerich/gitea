...
Distilled from gitea review rounds. Use them to identify the governing principle, not as text templates. This file deliberately quotes leaked wording as calibration material — the [recall batteries](recall-batteries.md) exclude the skill's directory, and its wording is not a license elsewhere.
...
**Fixed:** "Slash input resolves against the visible catalog — see the plain-text-reference design doc at [docs/development.md](../../../docs/development.md)."

The ordinal resolves nowhere at HEAD; the doc path and section do. Cite the owning doc at least once per file, as a relative link where the surface supports one.
...
**Leaked:** "Layering follows the design (v2 §3.2): `routers/api/v1/` is the pure core."

**Fixed:** "Layering: `routers/api/v1/` is the pure core."
...
**Leaked:** "`routers/api/v1/` is the shell (T4); the P-I migration owns the adapters."

**Fixed:** "`routers/api/v1/` is the shell; the adapters live in `routers/api/v1/repo/`."
...
Durable prose cannot see the stack. Keep the extension-point contract; the pending work's home is the PR itself, a `TODO`, or an issue referenced by full URL.

### "This PR" in a `docs/` page

**Leaked:** "This PR adds cursor-based pagination to the session list."

**Fixed:** "The session list paginates by cursor."

A `docs/` page outlives every PR; state the mechanism as current fact.
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and its issue.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside a design doc's change-story section ("the first cut shipped X") is current-state-safe; the indexical form never is.
...
**Fixed (in a design doc's Alternatives considered):** "**Caching the resolved spec.** Rejected: the spec depends on per-call cwd, so a cache keyed by request would serve stale roots."

The alternatives-considered genre is the sanctioned home; the reviewer and the round are not part of the rationale.

### Draft ordinals

**Leaked:** "As of v5 of this doc, the loader also validates manifests."

**Fixed:** "The loader validates manifests."

An implemented doc states shipped reality; its own revision history lives in git.
...
**Fixed:** (deleted; the wrapping order is stated in the function's Go doc comment.)
...
func RenderLabel(s string) template.HTML {
```

**Fixed:** (deleted; the three lines below the comment say the same thing in code.)
...
**Leaked:** "// The renderer runs on the client 端; see the 设计稿 for spacing."

**Fixed:** "// The renderer runs on the client side; spacing follows the design's spec."

Working-language fragments are transcription residue. When the prose is otherwise English, translate or delete. If the foreign term names a design artifact that resolves outside the repo, keep it as the canonical reference.
...
**Suspect:** An exported Go doc comment says "available for now," and a generator copies that sentence into the swagger spec.

**Wrong:** Rewrite only the source sentence during a prose-only purge, or hand-edit only the generated catalog.

**Right:** Trace the source's generated fan-out, update the owner, regenerate every derivative, and update the owning test. For a UI string in `options/locale/locale_en-US.json`, include the repository-required behavior evidence. If the authorized scope has no owning scenario, leave the wording unchanged and report the deferral.

## Keeps

### Issue references are durable on every surface

**Keep:** "The cap applies to the complete rendered value, wrappers included (issue <https://github.com/go-gitea/gitea/issues/1470> owns the follow-up)."

An unaided pass deleted this, reasoning that issue citations belong only in design docs. Wrong direction: issues resolve at HEAD from any surface, and "<URL> owns the follow-up" is the sanctioned home for deferred work in `docs/*.md`. Per `AGENTS.md`, reference issues and PRs by full URL, not by number.
...
**Keep (after fixing):** `//nolint:gosec // G101 — false positive: the substring is matched against a Go regex constant, never executed.`
...
**Keep:** A test that asks "what is today's date?" uses natural time, not a repository version stamp.

The prompt asks about the runtime clock; "today" does not contrast repository states. Because the text reaches a user, any rewrite still requires its owning behavior evidence.

## Overcorrection traps

Every trap below shipped in earlier review rounds. Enumerate a passage's propositions before trimming it.
...
**Original:** "The gate notice narrates the check order; the notice text is also what `make checks-backend` compiles against."

**Overcorrected:** "…" (whole sentence deleted as narration.)

**Right:** "The notice text is what `make checks-backend` compiles against."
...
**Original:** "The 4 MiB ceiling is measured: the largest generated swagger bundle is 3.1 MiB."

**Overcorrected:** "The ceiling is 4 MiB; the largest generated swagger bundle is 3.1 MiB."
