---
name: k7-merging-stacked-prs
description: Use when landing a stack of dependent GitHub PRs in the gitea repo, merging a PR whose base is another open PR's branch, or whenever a request mentions 'stacked PRs', 'PR stack', 'dependent PRs', or merging several related PRs in sequence.
---

# Landing an official GitHub PR stack

Land dependent PRs through GitHub's native stack object and `gh stack merge`. Do not reproduce stack semantics by merging and retargeting individual PRs with `gh pr merge` and `gh pr edit`. The root [AGENTS.md](../../../AGENTS.md) owns Conventional Commits, PR description discipline, and authorship trailers; [k7-pre-push-checks](../k7-pre-push-checks/SKILL.md) owns how the checks for each layer are selected. There are no installed git hooks in this repo, so gitea's GitHub Actions CI is the authority for every gate the stack must pass.

## Require the official stack extension

The `gh stack` subcommand ships as an official GitHub CLI extension, not a built-in. Verify it is installed before changing GitHub state; hard-stop if it is not and do not fall back to merging and retargeting PRs one at a time.
...
GitHub stacks require every head branch to live in the same repository, so hard-stop on a cross-fork chain. Use a clean dedicated worktree. Fetch current PR metadata and exact head OIDs rather than trusting branch names or an earlier report:
...
- [ ] Native `gh stack` extension is installed; every PR branch is in the same repository.
...
- [ ] Any rewritten layers passed relevant validation from [k7-pre-push-checks](../k7-pre-push-checks/SKILL.md), and review threads, approvals, mergeability, and checks were re-audited afterward.
